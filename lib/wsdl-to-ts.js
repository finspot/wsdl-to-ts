...
const wsdlSchemaGroupsToTypeString = (schema, keys) => keys
    .map((key) => {
    ...
    .join(EMPTY_LINE);
const wsdlSchemaToTypeString = (schemas) => Object.keys(schemas)
...
function wsdlTypeToInterfaceObj(obj, typeCollector) {
    ...
    for (const k of Object.keys(obj)) {
        ...
        if (t === "string") {
            ...
            else if (exports.nsEnums[typeFullName] || typeData) {
...
function wsdl2ts(wsdlUri, opts) {
    return new Promise((resolve, reject) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            wsdl_1.open_wsdl(wsdlUri, (error, wsdl) => {
                if (error) {
                    reject(error);
                }
                out.types.Common = {
...
function cloneObj(a) {
    ...
    for (const k of Object.keys(a)) {
        const t = typeof a[k];
        b[k] = t === "object" ? (Array.isArray(a[k]) ? a[k].slice() : cloneObj(a[k])) : a[k];
    }
    return b;
}
...
const getTypes = (str) => (str.match(/: \b[^;]*\b/g) || []).map((match) => match.substr(2)).filter((type) => !tsTypes[type]);
const uniq = (arr) => Object.keys(arr.reduce((out, curr) => (Object.assign({}, out, { [curr]: true })), {}));
